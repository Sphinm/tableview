  ...
  return (
    <div className={`relative flex items-center w-full ${containerClassName}`}>
      ...
      <input
        ...
        onBlur={handleBlur}
        disabled={disabled}
        className={`w-full ${prefix ? 'pl-9' : 'pl-3'} ${suffix ? 'pr-8' : 'pr-3'} py-2 bg-slate-950 border border-slate-800 rounded-xl text-sm font-semibold text-slate-100 focus:border-indigo-500 focus:outline-none transition-colors ${className}`}
        {...rest}
      />
      {suffix && (
