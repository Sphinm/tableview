  ...
  return (
    <div className={`relative flex items-center w-full ${containerClassName}`}>
      ...
      <input
        ...
        onBlur={handleBlur}
        disabled={disabled}
        className={`w-full ${prefix ? 'pl-9' : 'pl-3'} ${suffix ? 'pr-8' : 'pr-3'} py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-colors shadow-2xs ${className}`}
        {...rest}
      />
      {suffix && (
