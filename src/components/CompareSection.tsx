  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 sm:py-24 border-t border-slate-200">
      ...
      <div className="text-center max-w-3xl mx-auto mb-14">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold bg-white border border-slate-300 text-slate-900 mb-3.5 shadow-2xs">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-xs">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[700px]">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50 text-xs text-slate-800">
                <th className="p-4 sm:p-5 font-bold w-1/4">Evaluation Criteria</th>
                ...
                <th className="p-4 sm:p-5 font-bold w-1/6 text-slate-800">Python / Pandas</th>
                <th className="p-4 sm:p-5 font-bold w-1/6 text-slate-800">Desktop GUI (DBeaver)</th>
                <th className="p-4 sm:p-5 font-bold w-1/6 text-slate-800">Cloud Web Converters</th>
              </tr>
            </thead>
              ...
              {comparisonData.map((row, idx) => (
                <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4 sm:p-5 text-slate-800 font-medium">
                    <div className="flex items-center gap-2">
                      {row.pandas.icon && <row.pandas.icon className="size-4 text-slate-700 shrink-0" />}
                  ...
                  <td className="p-4 sm:p-5 text-slate-800 font-medium">
                    <div className="flex items-center gap-2">
                      {row.desktop.icon && <row.desktop.icon className="size-4 text-slate-700 shrink-0" />}
                      <span>{row.desktop.text}</span>
                    </div>
                  </td>
                  {/* Cloud Converters Column */}
                  <td className={`p-4 sm:p-5 ${row.cloud.highlightBad ? 'text-red-700 font-semibold' : 'text-slate-800 font-medium'}`}>
                    <div className="flex items-center gap-2">
                      {row.cloud.icon && (
                        <row.cloud.icon className={`size-4 shrink-0 ${row.cloud.highlightBad ? 'text-red-600' : 'text-slate-700'}`} />
                      )}
