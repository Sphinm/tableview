...
> | **加州 ARL 修正生效日** | ✅ 已复核：**加州总检察长**官方通稿（oag.ca.gov，2025-09-04）原文「amended, effective **July 1, 2025**」—— 初稿沿用的「2023-07-01」**有误，已更正** |
> | **第八巡回法院案号** | ✅ 已复核：CourtListener 记录为 **No. 24-3137, 24-3388**，*Custom Communications, Inc. v. FTC*，2025-07-08 判决 —— 研究 v1 的「24-2270」**有误，已由 v2 更正** |
> | **消费者法义务是否转移给 MoR** | ✅ **已修正本方案初稿的错误判断**：因 MoR 是法律上的卖方（合同相对方），面向消费者的法定义务**主要转移给 MoR**；但不可放弃的救济（DCD Art.22 / ACL s.64）、产品符合性、自有站点隐私义务、本国所得税**仍归你**（§8.3）
...
| 美国州法（加州 ARL 等） | **仍然有效，且有新修正** ✅（B&P Code §17602/§17603；click-to-cancel 修正为 **AB 2863**，**2025-07-01 起适用**——已由**加州总检察长**官方通稿核实） | 自动续订需披露续订条款与价格；取消须「exclusively online, at will, and **without engaging any further steps that obstruct or delay**」；**由 MoR 主要承担**（§8.3） |
...
### 8.3 MoR 转移了什么、没转移什么（**本节已修正一处重要错误** ✅）

> ⚠️ **修正记录**：本节初稿把「消费者法信息义务（撤回权告知、续订披露、取消便利）」列入「MoR **不**接管」。
> 经复核（合规研究 v2 + 一手来源）确认**该判断错误并已更正**：因为 MoR 是**法律上的卖方**（消费者的**合同相对方**，即法条中的 "trader"），这些**面向消费者的法定信息与流程义务主要落在 MoR 头上** —— 这是**真正的转移**，不只是税。
> 这实际**减轻了你的工作量**（弃权流程由 MoR 的托管结账页实现），但也意味着**你必须验证 MoR 确实做了**，而不是默认它做了。

| MoR **主要接管** | 仍然**跑不掉**（无论谁来卖） |
| :--- | :--- |
| **卖方 of record** 身份（FastSpring 原文："seller and merchant of record"；Paddle 为 "authorised reseller"，客户 "purchase the Product from Paddle"） | **你本国的所得税**（无任何 MoR 声明代缴；且仍会向你收集税务身份，如 W-8 / IRS 信息） |
| 间接税注册/计算/代收/申报/缴纳 | **产品符合性／质量责任** —— 沿供应链追到作为**供应方**的你（FastSpring 明示：交易用其条款，但「we recommend you still apply your own **EULA**」） |
| 卡合规（PCI / SCA / 3DS） | **不可由合同排除的消费者救济**（EU DCD **Art.22**、ACL **s.64(1)「void」**）—— 换卖方也不会消失 |
| **面向消费者的法定义务**：撤回权告知与**弃权流程**、DCD/CRA/ACL 退款、ARL 披露与取消 | **你自有网站/品牌的 GDPR·CCPA 告知义务**（Paddle 的 DPA 是 **controller-to-controller**：「Each party shall bear responsibility for **its own compliance obligations**」） |
| 退款/拒付的**流程处理** | **拒付成本**仍从你的分成中扣除；**你的站点与品牌**仍是监管与争议的抓手 |

> **最需要纠正的直觉**：MoR **不止**买「税 + 卖方身份 + 卡合规」，它**也买了面向消费者的合同义务**；但它**不是**「全部合规外包」—— 上面右列四项与你的**自有站点**始终是你的。

### 8.4 消费者法：谁做什么 + 本方案的设计要求

**由 MoR 实现（但你要在合同中确认它确实做了）**：

- **欧盟 14 天撤回权**（CRD Art.9(1) ✅ 逐字）：数字内容即时交付须取得消费者 **(a) 事先明示同意** 与 **(b) 确认知悉丧失撤回权**（Art.16(m) ✅）。**MoR 的托管结账页是承载该弃权流程的地方**，不是你的落地页。
  - 佐证其严肃性：若未履行 Art.6(1)(h) 告知 → 撤回期**延长至 12 个月**（Art.10）。
- **英国等同要求**（CCR 2013）：reg.30(2) 14 天；reg.37(1) 未经明示同意+知悉**不得开始供应**；**reg.37(4)：未取得同意或未以耐久介质确认 → 消费者对已交付数字内容「不承担任何费用」**（即**收不到钱**）。这是最强的一条经济风险，也正是**必须选一个合规实现可靠的 MoR** 的理由。
- **退款**：DCD Art.16(1) 退还全部款项、**Art.18(1) 14 天内退款**；ACL s.267(3)；UK CRA **s.45(3) 14 天内退款**。

**仍必须由你做的（设计要求）**：

- **验证而非假设**：在 MoR 合同/沙箱中**实测**弃权流程、退款时限与 ARL 取消路径确实存在
- **你的 ToS 与退款政策**须与 MoR 的做法**一致**（不能一边写"不退款"一边由 MoR 退款）
- **产品符合性**：你交付的数字内容须与描述相符（这是追到你身上的那条）
- **PCI DSS**：**托管结账**，卡数据**永不**经我方服务器 → **SAQ A**；**禁止**存储卡号/CVV（✅ PCI SSC 一手）
...
**A. 由 MoR 实现 —— 你的任务是「验证」而不是「实现」（§8.3）**

- [ ] 在 MoR **沙箱实测**：数字内容弃权流程（明示同意 + 知悉丧失撤回权）确实出现在结账页（EU/UK）
- [ ] 实测：退款时限符合 DCD Art.18(1) / UK CRA s.45(3) 的 **14 天**
- [ ] 实测：取消路径符合加州 ARL 的「exclusively online, at will, without further steps that obstruct or delay」
- [ ] 从 MoR 取得 DPA，并确认其承担间接税与面向消费者的合同义务（写入合同）

**B. 仍必须你自己做**

- [ ] **你的 ToS/退款政策与 MoR 实际做法一致**（不可自相矛盾）
- [ ] **不**声称「依 FTC click-to-cancel」（该规则已撤销，§8.2）
- [ ] 卡片数据 100% 由渠道托管（验证：我方服务器**无任何卡字段**）
- [ ] 隐私政策更新；评审 GDPR **Art.27** 欧盟代表义务；与渠道/Google/邮件签 DPA
